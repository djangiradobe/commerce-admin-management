...
function toClientShape(creds) {
    if (!creds)
        return null;
    if (normalizeConnectionType(creds.connectionType) !== CONNECTION_TYPE_PAAS)
        return null;
    return {
...
function toSaasClientShape(creds) {
    if (!creds)
        return null;
    if (normalizeConnectionType(creds.connectionType) !== CONNECTION_TYPE_SAAS)
        return null;
    return {
        url: normalizeBaseUrl(creds.baseUrl),
        apiKey: creds.apiKey || ''
...
function jwtExpMs(token) {
    try {
        ...
        if (claims && claims.created_at && claims.expires_in)
            return Number(claims.created_at) + Number(claims.expires_in);
    }
    catch (_) { }
    return 0;
}
...
async function mintSaasBearerToken(params) {
    ...
    if (!scopes.length)
        scopes = parseScopes(params.IMS_OAUTH_S2S_SCOPES);
    if (!scopes.length)
        scopes = DEFAULT_ACCS_SCOPES.slice();
    if (!scopes.includes('commerce.accs'))
        scopes.push('commerce.accs');
...
async function writeCommerceCreds(params, creds) {
    ...
    try {
        ...
        try {
            await collection.updateOne({ _id: DOC_ID }, {
                ...
                try {
                    ...
                    try {
                        ...
                        detail = parsed.message
                            ...
                            : body;
                        if (parsed.parameters && parsed.parameters.resources)
                            aclResource = parsed.parameters.resources;
                    }
                    catch (_) {
                        detail = body;
                    }
...
async function getCommerceCreds(params, logger) {
    ...
    if (fromEnv.baseUrl && fromEnv.consumerKey)
        return fromEnv;
