...
function validateSchema(schema) {
    ...
    for (const section of schema.sections) {
        ...
        if (!Array.isArray(section.groups))
            throw new Error(`section "${section.id}": groups must be array`);
    ...
    try {
        ...
        if (op === 'save' || op === 'reset' || op === 'import') {
            if (rbacHook && rbacHook.assertMinRole) {
                ...
                if (roleErr)
                    return errorResponse(403, roleErr, logger);
            }
            else {
        ...
        if (op === 'save') {
            ...
            const removedPaths = new Set([...prevPaths].filter((p) => !nextPaths.has(p)));
            ...
            // with the user first.
            const cascadeConfirmed = params.confirmCascade === true || params.confirmCascade === 'true';
            ...
            let writeResult;
            if (existing) {
                writeResult = await collection.updateOne({ _id: DOC_ID }, { $set: { schema: params.schema, updatedAt: now } });
            }
            else {
            ...
            logger.info(`Schema saved (existing=${!!existing}, removed=${removedPaths.size}, result=${JSON.stringify(writeResult)})`);
            ...
            if (!verify || !verify.schema) {
                return errorResponse(500, 'Schema write completed but document is missing — check ABDB region/permissions', logger);
            }
