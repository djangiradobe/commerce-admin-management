...
async function fetchTargetStoreMappingsFromCommerce(params, logger) {
    ...
    try {
        ...
        if (logger)
            logger.warn(`Commerce REST lookup failed during import remap: ${err.message}`);
        return null;
    }
}
/**
...
async function resolveTargetMappings(params, client, logger) {
    ...
    if (fromCommerce)
        return { mapping: fromCommerce, source: 'commerce' };
    const fromAbdb = await readTargetStoreMappingsFromAbdb(client);
    if (fromAbdb)
        return { mapping: fromAbdb, source: 'abdb' };
...
function buildIdMap(source, target) {
    ...
    const indexSide = (mapping, websiteByCode, storeByCode) => {
        ...
        for (const [storeId, m] of Object.entries(mapping)) {
            ...
            if (m.website_code && m.website_id != null) {
                ...
                if (!websiteByCode.has(m.website_code))
                    websiteByCode.set(m.website_code, String(m.website_id));
            }
            if (m.code)
                storeByCode.set(m.code, String(storeId));
        }
    };
    ...
    for (const [code, srcId] of websiteSrcByCode.entries()) {
        ...
        if (websiteTgtByCode.has(code))
            websites[srcId] = websiteTgtByCode.get(code);
    }
    const stores = {}; // sourceStoreId → targetStoreId
    const storeCodes = {};
    for (const [code, srcId] of storeSrcByCode.entries()) {
        storeCodes[srcId] = code;
        if (storeTgtByCode.has(code))
            stores[srcId] = storeTgtByCode.get(code);
    }
...
async function upsertOne(collection, doc, { overwrite }) {
    ...
    try {
        ...
        const msg = err && err.message ? String(err.message) : String(err);
        if (!/upsert|unsupported|not implemented/i.test(msg))
            throw err;
    ...
    try {
        ...
        if (!valuesOnly && schemaIn && typeof schemaIn === 'object' && Array.isArray(schemaIn.sections)) {
            ...
            if (existing && !overwrite) {
                ...
                try {
                    await schemaCol.updateOne({ _id: SCHEMA_DOC_ID }, {
        ...
        if (!schemaOnly && Array.isArray(valuesIn)) {
                    ...
                    for (const s of schemaForFlags.sections) {
                        for (const g of (s.groups || [])) {
                            for (const f of (g.fields || [])) {
                                if (f && f.sensitive)
                                    sensitivePathSet.add(`${s.id}/${g.id}/${f.id}`);
                            }
                        }
                    }
                }
            ...
            const { mapping: targetMappings, source: targetSource } = await resolveTargetMappings(params, client, logger);
            ...
            if (targetMappings) {
                for (const [storeId, m] of Object.entries(targetMappings)) {
                    ...
                    if (m.code)
                        targetStoreIdByCode.set(String(m.code), String(storeId));
                }
            }
            summary.idMap = {
                targetSource,
            ...
            logger.info(`target Commerce (${targetSource}): ` +
            ...
            for (const row of valuesIn) {
                ...
                if (r === 'inserted')
                    ...
                    summary.valuesSkipped++;
                else
                    summary.valuesUpserted++;
            }
        }
