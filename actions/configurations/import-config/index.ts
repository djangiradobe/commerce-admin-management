...
function buildIdMap (source, target) {
  ...
  const indexSide = (mapping, websiteByCode, storeByCode) => {
    ...
    for (const [storeId, m] of Object.entries(mapping) as [string, any][]) {
...
export async function main (params) {
  ...
  try {
    ...
    if (!schemaOnly && Array.isArray(valuesIn)) {
      ...
      if (targetMappings) {
        for (const [storeId, m] of Object.entries(targetMappings) as [string, any][]) {
          if (!m) continue
      ...
      const translateScopeId = (scope, srcId, scopeCode) => {
        ...
        if (scope === 'websites') {
          ...
          if (targetMappings && Object.values(targetMappings).some((m: any) => m && String(m.website_id) === s)) {
