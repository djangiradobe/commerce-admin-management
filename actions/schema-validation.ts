...
function validateFieldValue (field, value) {
  ...
  if (field.type === 'number') {
    ...
    if (Number.isNaN(n)) return `${fieldLabel(field)} must be a number`
    if (v.min != null && n < v.min) return `${fieldLabel(field)} must be ≥ ${v.min}`
    if (v.max != null && n > v.max) return `${fieldLabel(field)} must be ≤ ${v.max}`
...
function indexSchemaByPath (schema) {
  const idx = new Map()
  if (!schema || !Array.isArray(schema.sections)) return idx
