...
let _rbacHook = null;
try {
    _rbacHook = require('@adobedjangir/commerce-admin-ims-access/hook');
}
catch (_) {
    _rbacHook = null;
}
...
async function requireRole(params, minRole, opts = {}) {
    if (!_rbacHook || !_rbacHook.assertMinRole)
        return null; // no RBAC add-on → open
    try {
...
async function requireValidToken(params) {
    if (!_rbacHook || !_rbacHook.assertValidCaller)
        return null; // no RBAC add-on → open
    try {
...
async function resolveCallerRole(params) {
    if (!_rbacHook || !_rbacHook.resolveCallerRole)
        return null;
    try {
        return await _rbacHook.resolveCallerRole(params);
    }
    catch (_) {
        return null;
    }
