...
function updateExistingExtensionBlock(content) {
    ...
    const blockRe = new RegExp(`^${escapeRe(indent)}commerce/backend-ui/1:[ \\t]*\\n` +
        `(?:${escapeRe(includeIndent)}\\$include:[^\\n]*\\n)?`, 'm');
    const replacement = `${indent}${EXTENSION_POINT}:\n${includeIndent}$include: ${INCLUDE_REL}\n`;
...
function ensureHostDeps(projectRoot) {
    ...
    for (const [name, floor] of Object.entries(REQUIRED_HOST_DEPS)) {
        ...
        if (declared && satisfiesFloor(declared, floor))
            continue;
        bumped.push({ name, was: declared || '(missing)', now: floor });
    }
    if (bumped.length === 0)
        return { changed: false, reason: 'already-satisfies' };
...
function mirrorImsOauthAliases(projectRoot) {
    ...
    if (!fs.existsSync(envPath))
        return { changed: false, reason: 'no-env' };
    let env = '';
    try {
    ...
    if (!anyMissing)
        return { changed: false, reason: 'already-aliased' };
...
function ensureEnvDefaults(projectRoot) {
    ...
    if (additions.length === 0)
        return { changed: false, set: [], file: envPath };
...
function saasGuidance(projectRoot) {
    ...
    if (fs.existsSync(appCfg)) {
        try {
            hasExtPoint = /commerce\/extensibility\/1/.test(fs.readFileSync(appCfg, 'utf8'));
        }
        catch (_) { }
    }
...
function explicitPlatformChoice() {
    ...
    if (process.env.COMMERCE_PLATFORM)
        return String(process.env.COMMERCE_PLATFORM).toLowerCase();
    return null;
}
...
function promptPlatform(def) {
    ...
    return new Promise((resolve) => {
        ...
        const ask = () => rl.question(`Which Adobe Commerce platform is this app for? [paas/saas] (default: ${fallback}): `, (ans) => {
...
function writeCommerceAppConfig(projectRoot) {
    ...
    if (existing) {
        ...
        try {
            hasAdminUiSdk = /adminUiSdk\s*:/.test(fs.readFileSync(path.join(projectRoot, existing), 'utf8'));
        }
        catch (_) { }
        return { changed: false, file: existing, hasAdminUiSdk };
    }
    const file = 'app.commerce.config.ts';
    fs.writeFileSync(path.join(projectRoot, file), commerceAppConfigContents(readEnvValue(projectRoot, 'APP_TITLE'), readEnvValue(projectRoot, 'APP_SECTION_TITLE')), 'utf8');
...
function wireSaasAppConfig(projectRoot) {
    ...
    let YAML;
    try {
        YAML = require(require.resolve('yaml', { paths: [projectRoot] }));
    }
    catch (_) {
...
function unwireSaasAppConfig(projectRoot) {
    ...
    if (doc.hasIn(BE)) {
        if (doc.getIn([...BE, '$include']) !== INCLUDE_REL) {
            doc.setIn([...BE, '$include'], INCLUDE_REL);
            changes.push('backend-ui→paas');
        }
        if (doc.getIn([...BE, 'hooks', 'pre-app-build']) !== DISCOVER_CMD) {
            doc.setIn([...BE, 'hooks', 'pre-app-build'], DISCOVER_CMD);
            changes.push('backend-ui-hook');
        }
    }
    ...
    if (doc.hasIn(['application', 'runtimeManifest', 'database'])) {
        doc.deleteIn(['application', 'runtimeManifest', 'database']);
        changes.push('-application-database');
    }
    // 4. Drop the application discover hook (it moves back to backend-ui/1).
    if (doc.getIn(['application', 'hooks', 'pre-app-build']) === DISCOVER_CMD) {
        doc.deleteIn(['application', 'hooks', 'pre-app-build']);
        changes.push('-application-hook');
        const hooks = doc.getIn(['application', 'hooks']);
        if (hooks && hooks.items && hooks.items.length === 0)
            doc.deleteIn(['application', 'hooks']);
    }
    // 5. Remove the App Management extension so nothing SaaS-only deploys.
    if (doc.hasIn(['extensions', 'commerce/extensibility/1'])) {
        doc.deleteIn(['extensions', 'commerce/extensibility/1']);
        changes.push('-extensibility');
    }
    if (changes.length === 0)
        return { changed: false, reason: 'already-paas' };
    fs.writeFileSync(p, doc.toString(), 'utf8');
    return { changed: true, changes };
}
...
function main() {
    ...
    if (process.env.CONFIGURATION_MANAGEMENT_SKIP_SETUP === '1')
        return;
    if (process.env.COMMERCE_ADMIN_MANAGEMENT_SKIP_SETUP === '1')
        return;
    const projectRoot = resolveProjectRoot();
    if (!projectRoot) {
        console.log('[@adobedjangir/commerce-admin-management] No App Builder project found — skip setup. ' +
    ...
    if (app.changed) {
        console.log(`[@adobedjangir/commerce-admin-management] Updated app.config.yaml (${app.reason}):\n` +
            `  $include: ${app.detail}`);
    }
    else {
...
async function runCli() {
    ...
    if (platform)
        setEnvVar(projectRoot, 'COMMERCE_PLATFORM', platform);
    // Explicit CLI transitions:
    //   • saas → wire App Management.
    //   • paas → cleanly revert if the project is currently in SaaS layout.
    if (interactive) {
        if (platform === 'saas')
            enableSaas(projectRoot);
        else if (platform === 'paas')
            disableSaas(projectRoot);
    }
}
if (require.main === module) {
    runCli().catch((err) => {
        ...
        console.error('[@adobedjangir/commerce-admin-management] setup encountered an error ' +
